...
```
(No `standalone` target — see plan.md item 4.)

Or skip remembering these — `gui.bat` launches a desktop GUI covering build,
CARLA, and replay/Foxglove (see [docs/df_dev_gui_plan.md](../../docs/df_dev_gui_plan.md), superproject root).

## Layout

- `src/component/common/` — shared framework (`df_common` lib): ports, `IDfFunction`, `DfRunner`, `ParamLoader` (Step 3)
...
- `src/platform/standalone/` — not used for `df` (plan.md item 4)
